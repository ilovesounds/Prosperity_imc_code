...
def flatten_near_close(
    ...
    orders: List[Order] = []
    if abs(position) == 0 or timestamp < start_ts:
        return orders
    ...
    urgency = 0.35 if timestamp < 98500 else 0.7
    qty = max(1, min(abs(position), int(math.ceil(abs(position) * urgency))))

    if position > 0 and bid is not None:
        orders.append(Order(product, bid, -min(qty, sell_capacity(product, position))))
    elif position < 0 and ask is not None:
        orders.append(Order(product, ask, min(qty, buy_capacity(product, position))))

    return orders


def high_strike_sell_strategy(
    product: str,
    depth: OrderDepth,
    position: int,
    timestamp: int,
    ...
    fair = DEFAULT_FAIRS[product]

    # These names show repeat buyer flow lifting Mark 22 around the bid.
    # Prefer sell-biased participation over symmetric market making.
    if position > 0:
        exit_px = bid if bid is not None else ask
        if exit_px is not None:
            qty = min(position, 8 if timestamp < 98000 else position)
            if qty > 0:
                orders.append(Order(product, exit_px, -qty))
                position -= qty

    if bid is not None and bid >= fair - 1:
        sell_px = bid
        qty = min(6, sell_capacity(product, position))
        if qty > 0:
            orders.append(Order(product, sell_px, -qty))

    if position < -40 and ask is not None:
        cover_qty = min(-position - 40, buy_capacity(product, position), 4)
        if cover_qty > 0:
            orders.append(Order(product, ask, cover_qty))
...
def legacy_option_quotes(
    ...
    add_passive_quotes(orders, product, depth, position, buy_px, sell_px, quote_lot)
    orders.extend(flatten_near_close(product, depth, position, timestamp, start_ts=98500))
    return orders
...
class Trader:
    def run(self, state: TradingState):
        ...
        if HYDROGEL in state.order_depths:
            ...
            hydro_orders = quote_inside_spread(
                HYDROGEL,
        ...
        if VELVET in state.order_depths:
            ...
            velvet_orders = fair_value_quotes(
                VELVET,
        ...
        for product in OPTION_CONFIGS:
            ...
            if product in {"VEV_5300", "VEV_5400", "VEV_5500"}:
                orders = high_strike_sell_strategy(product, depth, position, state.timestamp)
                orders.extend(flatten_near_close(product, depth, position, state.timestamp, start_ts=98000))
            else:
                fair = st.get_fair(product, DEFAULT_FAIRS[product])
                orders = legacy_option_quotes(product, depth, position, fair, state.timestamp)
            if orders:
